...
struct ModelArgs {
    ...
    /// Attention implementation; flash requires --features flash-attn at build time.
    #[arg(long, default_value = "native", value_parser = ["native", "flash"])]
...
mod native {
    ...
    use nano_vllm_rs::{
        model::{AttentionBackend, ModelOptions, QwenModel},
    ...
    fn load_engine(args: &ModelArgs) -> Result<(LlmEngine<QwenModel>, Value, f64)> {
        ...
        let model = QwenModel::load(
            ...
            ModelOptions {
                ...
                attention_backend: match args.attention_backend.as_str() {
                    "flash" => AttentionBackend::Flash,
                    _ => AttentionBackend::Native,
                },
            },
        )?;
