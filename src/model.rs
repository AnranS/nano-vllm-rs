...
impl Default for ModelOptions {
    fn default() -> Self {
        Self {
            ...
            max_model_len: 4096,
            cuda_graphs: true,
            attention_backend: AttentionBackend::Native,
        }
    }
}
...
struct FlashWorkspace {
    ...
    lse_accum: Buffer,
    out_accum: Buffer,
    decode_splits: Vec<usize>,
}

/// FlashAttention consumes grouped query suffixes, including cached-prefix and
/// chunked prefill offsets. Validate on the host before exposing device metadata.
fn flash_metadata(batch: &Batch, sequences: usize) -> Result<(Vec<i32>, Vec<i32>, usize)> {
    let mut cu_q = vec![0];
    let mut kv_lens = Vec::with_capacity(sequences);
    let mut row = 0;
    let mut max_q = 0;
    for sequence in 0..sequences {
        let start = row;
        while row < batch.sequence_ids.len() && batch.sequence_ids[row] == sequence as i32 {
            if row > start {
                ensure!(
                    batch.positions[row] == batch.positions[row - 1] + 1,
                    "FlashAttention queries must have consecutive positions"
                );
            }
            row += 1;
        }
            ...
            row > start,
            "FlashAttention requires nonempty query groups in sequence order"
        );
        max_q = max_q.max(row - start);
        cu_q.push(row.try_into().context("query offset exceeds i32")?);
        kv_lens.push(
            batch.positions[row - 1]
                .checked_add(1)
                .context("KV length overflow")?,
        );
    }
    ensure!(
        row == batch.sequence_ids.len(),
        "FlashAttention query groups are out of order"
    );
    Ok((cu_q, kv_lens, max_q))
}
    ...
    fn new(tokens: usize, sequences: usize, table_stride: usize) -> Result<Self> {
        ...
        Ok(Self {
            ...
            cu_q: segment(
                sequences
                    .checked_add(1)
                    .context("sequence count overflow")?,
                4,
            )?,
            kv_lens: segment(sequences, 4)?,
            bytes: cursor,
        })
    }
...
pub struct QwenModel {
    ...
    // Captured kernels trust the capacities used at allocation time.
...
impl QwenModel {
    pub fn load(path: &Path, options: ModelOptions) -> Result<Self> {
        ...
        let decode_splits = if options.attention_backend == AttentionBackend::Flash {
            ensure!(
                cfg!(feature = "flash-attn"),
                "flash backend requires cargo build --features flash-attn"
            );
            ensure!(
                config.head_dim == 128
                    && config.num_attention_heads <= 256
                    && options.block_size.is_multiple_of(256),
                "flash backend requires head_dim=128, query heads<=256 and block_size divisible by 256"
            );
            let sms = gpu.sm_count()?;
            let mut splits = vec![1];
            for batch in 1..=options.max_sequences {
                splits.push(gpu.flash_num_splits(
                    batch,
        ...
        let work = Workspace {
            ...
            flash: if let Some(decode_splits) = decode_splits {
                let splits = *decode_splits.iter().max().unwrap_or(&1);
                Some(FlashWorkspace {
                    cu_q: view(&metadata_layout.cu_q)?,
                    kv_lens: view(&metadata_layout.kv_lens)?,
                    lse: alloc(&[s, config.num_attention_heads, t], 4)?,
                    lse_accum: alloc(&[splits, s, config.num_attention_heads], 4)?,
                    out_accum: alloc(&[splits, s, config.num_attention_heads, 128], 4)?,
                    decode_splits,
                })
            } else {
                None
...
impl ModelRunner for QwenModel {
    fn run(&mut self, batch: &Batch) -> Result<Vec<u32>> {
        let (rows, sequences, samples) = self.validate_batch(batch)?;
            ...
            (self.options.attention_backend == AttentionBackend::Flash)
                == self.work.flash.is_some(),
            "attention backend cannot be changed after model loading"
        );
        self.last_sample_count = 0;
        self.metadata_layout
            .pack(&mut self.host_metadata, batch, sequences, self.table_stride);
        let (max_q, max_k) = if self.work.flash.is_some() {
            let (cu_q, kv_lens, max_q) = flash_metadata(batch, sequences)?;
            for (range, values) in [
                (&self.metadata_layout.cu_q, &cu_q),
                (&self.metadata_layout.kv_lens, &kv_lens),
            ] {
                for (i, value) in values.iter().enumerate() {
                    let start = range.start + i * 4;
                    self.host_metadata[start..start + 4].copy_from_slice(&value.to_ne_bytes());
                }
            }
            // Fix decode launch geometry for graph replay as KV lengths grow.
            let max_k = if max_q == 1 {
                self.options.max_model_len
            } else {
                *kv_lens.iter().max().context("empty KV lengths")? as usize
            };
            (max_q, max_k)
        } else {
            (0, 0)
        ...
        if self.options.cuda_graphs && batch.is_decode && rows == sequences && samples == sequences
        {
            if !self.graphs.contains_key(&rows) {
                ...
                self.forward(rows, sequences, samples, max_q, max_k)?;
                self.gpu.sync()?;
                let graph = self
                    .gpu
                    .capture(|| self.forward(rows, sequences, samples, max_q, max_k))?;
...
mod tests {
    ...
    #[test]
    fn flash_metadata_preserves_cached_prefix_causal_offsets() {
        let batch = Batch {
            positions: vec![256, 257, 258, 512, 513],
            sequence_ids: vec![0, 0, 0, 1, 1],
            ..Batch::default()
        };
        let (cu_q, kv_lens, max_q) = flash_metadata(&batch, 2).unwrap();
        assert_eq!(cu_q, vec![0, 3, 5]);
        assert_eq!(kv_lens, vec![259, 514]);
        assert_eq!(max_q, 3);
        // Bottom-right causal masking aligns query zero with the actual
        // cached-prefix length, rather than with absolute position zero.
        for sequence in 0..2 {
            let first = cu_q[sequence] as usize;
            let query_len = cu_q[sequence + 1] - cu_q[sequence];
            assert_eq!(kv_lens[sequence] - query_len, batch.positions[first]);
        }
    }

    #[test]
    fn flash_metadata_handles_ragged_prefill_and_single_token_decode() {
        let ragged = Batch {
            positions: vec![0, 1, 1023, 509, 510, 511],
            sequence_ids: vec![0, 0, 1, 2, 2, 2],
            ..Batch::default()
        };
        assert_eq!(
            flash_metadata(&ragged, 3).unwrap(),
            (vec![0, 2, 3, 6], vec![2, 1024, 512], 3)
        );
        let decode = Batch {
            positions: vec![0, 255, 4095],
            sequence_ids: vec![0, 1, 2],
            is_decode: true,
            ..Batch::default()
        };
        assert_eq!(
            flash_metadata(&decode, 3).unwrap(),
            (vec![0, 1, 2, 3], vec![1, 256, 4096], 1)
        );
    }

    #[test]
    fn flash_metadata_rejects_invalid_groups_and_nonconsecutive_positions() {
        for (positions, sequence_ids, sequences) in [
            (vec![0, 0, 1], vec![0, 1, 0], 2), // A group reappears later.
            (vec![0, 0], vec![1, 0], 2),       // Groups are out of order.
            (vec![0, 0], vec![0, 2], 3),       // Empty middle group.
            (vec![255, 257], vec![0, 0], 1),   // Missing query position.
            (vec![256, 256], vec![0, 0], 1),   // Repeated query position.
            (vec![i32::MAX], vec![0], 1),      // KV length would overflow.
        ] {
            let batch = Batch {
                positions,
                sequence_ids,
            ...
            assert!(flash_metadata(&batch, sequences).is_err());
        }
    }
